...
-- Ensure approved_by is VARCHAR(255) to support both UUID and mobile/text identifiers
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_profiles' AND column_name='approved_by') THEN
        ALTER TABLE public.user_profiles ALTER COLUMN approved_by TYPE VARCHAR(255) USING approved_by::text;
    END IF;
END $$;
